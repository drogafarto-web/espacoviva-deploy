...
def mpv_command(*args):
    try:
        ...
        # mpv pode mandar múltiplos eventos JSON. Pegamos apenas o primeiro objeto (a resposta ao comando).
        ...
        for line in resp.strip().split("\n"):
            ...
            try:
                ...
                if "request_id" in data or "error" in data: # É a resposta ao comando
                    log(f"Resposta mpv_command: {line[:100]}...")
                    return data
            except: continue
        return {}
...
<body>
  ...
  <div class="config-panel" id="cfg">
    ...
    <input type="text" id="api_url" placeholder="URL do Tunnel (ex: academia.trycloudflare.com)"
      style="margin: 10px 0;">
  ...
  <div class="container">
    <header>
      <div class="brand">
        <h1>Academia Espaço viva Infinite</h1>
      </div>
      <div class="sys-info">
        <div class="status-pill">
          <div class="dot" id="dotNet"></div> <span id="netText">---</span>
        </div>
    ...
    <div class="card">
      <div class="card-title">Busca Inteligente <span class="inf-mode"
          onclick="api('/infinite').then(refresh)">Auto-Refill</span></div>
      <div class="search-box">
        <input type="text" id="inp" placeholder="O que vamos ouvir hoje?"
          onkeydown="if(event.key==='Enter') add('track')">
    ...
    <div class="card" id="qCard" style="display:none">
      <div class="card-title">Fila de Reprodução <button class="btn btn-secondary" id="clearBtnQueue"
          style="padding: 4px 10px; font-size: 9px;" onclick="confirmClear(this)">LIMPAR</button></div>
  ...
  <script>
    ...
    function saveConfig() {
      let url = document.getElementById('api_url').value.trim();
    ...
    async function refresh() {
      ...
      const base = API_BASE || "";
      try {
        const s = await fetch(base + '/api/state?t=' + Date.now() + '&auth=' + auth, { mode: 'cors' }).then(r => r.json());
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def check_auth(self):
        ...
        # Also allow if it's in the query string for easier state polling
        if "auth=espacoviva" in self.path: return True
        return False
    ...
    def do_GET(self):
        if self.path == "/" or "/index.html" in self.path: 
            ...
            if not self.check_auth(): return self._unauthorized()
